...
const Game = () => {
  ...
  return (
    <div className="game-screen" style={{ backgroundImage: `url(${background})` }}>
      ...
      <div className="tree-wrapper">
        <img src={renderTreeImage()} alt="left-tree" className={`tree-overlay ${treeState === 'fall' ? 'fall-left' : ''}`} />
        <img src={renderTreeImage()} alt="right-tree" className={`tree-overlay ${treeState === 'fall' ? 'fall-left' : ''}`} />
      </div>

      {isPicked && <img src={trashcan} alt="trashcan" className="trashcan" />}
      <img
        src={trashImg}
        alt="trash"
        className={`trash ${isThrown ? 'throw' : ''} ${isPicked ? 'pick' : ''}`}
      />
